...
""")
    ...
    # Soft gate: custom branding requires Pro Kit
    from ..utils.license import is_pro_key
    from ..utils.messaging import show_branding_gate_message
    if not is_pro_key(license_key):
        if manifest.config.brand_color and manifest.config.brand_color != DEFAULT_BRAND_COLOR:
            show_branding_gate_message()
        if manifest.config.logo_path:
            show_branding_gate_message()
        brand_color = DEFAULT_BRAND_COLOR
        logo_path = None
    else:
        brand_color = manifest.config.brand_color or DEFAULT_BRAND_COLOR
        logo_path = manifest.config.logo_path
