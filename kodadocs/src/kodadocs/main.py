...
def _load_dotenv(path: Path):
    """Load .env file from project path into os.environ if present."""
    env_file = path / ".env"
...
from kodadocs import __version__ as VERSION  # noqa: E402
...
def main(
    ...
    if ctx.invoked_subcommand is None:
        ...
        console.print("  Claude Code MCP tool for generating help documentation")
        console.print()
        ...
        console.print()
        console.print(
            '  Then tell Claude: [green]"Generate help docs for my app"[/green]'
        )
        console.print()
...
def init(
    ...
    output: Path = typer.Option(
        Path("./docs"), help="Path to save generated documentation"
    ),
):
    """Advanced setup wizard. Optional — 'kodadocs generate' works without this."""
    console.print(
        f"[bold green]Initializing KodaDocs for {path.absolute()}[/bold green]"
    )
    ...
    _load_dotenv(path.resolve())

    app_url = Prompt.ask(
        "What is the URL of your app (for screenshots)?",
    ...
    include_patterns = Prompt.ask(
        "Include patterns (comma separated)", default="**/*"
    ).split(",")
    exclude_patterns = Prompt.ask(
        "Exclude patterns (comma separated)", default="node_modules/**, .git/**"
    ).split(",")
    ...
    if not Confirm.ask(
        f"Is [cyan]{detected_framework.value}[/cyan] the correct framework?",
        default=True,
    ):
        framework_options = [f.value for f in Framework]
        framework_value = Prompt.ask(
            "Select the correct framework",
            choices=framework_options,
            default=Framework.UNKNOWN.value,
        )
...
def config(
    show: bool = typer.Option(
        False, "--show", "-s", help="Pretty-print current configuration"
    ),
    url: Optional[str] = typer.Option(None, "--url", help="Update app URL"),
    brand_color: Optional[str] = typer.Option(
        None, "--brand-color", help="Update brand color (hex)"
    ),
    ...
    framework: Optional[str] = typer.Option(
        None, "--framework", help="Update framework"
    ),
    ...
    has_updates = any(
        v is not None for v in [url, brand_color, logo, model, output, framework]
    )

    if not config_file.exists():
        console.print(
            "[yellow]No config found. Run [bold]kodadocs init[/bold] or [bold]kodadocs generate[/bold] first.[/yellow]"
        )
        raise typer.Exit(code=1)
    ...
    if has_updates:
        ...
        if framework is not None:
            ...
            if framework not in valid:
                console.print(
                    f"[red]Invalid framework '[bold]{framework}[/bold]'. Choose from: {', '.join(valid)}[/red]"
                )
                raise typer.Exit(code=1)
    ...
    if not has_updates or show:
        ...
        console.print(
            f"  framework:   {cfg.framework.value if isinstance(cfg.framework, Framework) else cfg.framework}"
        )
...
def generate(
    path: Path = typer.Argument(
        Path("."), help="Project path to run KodaDocs pipeline"
    ),
    url: Optional[str] = typer.Option(
        None, "--url", "-u", help="Override app URL for screenshots"
    ),
    fresh: bool = typer.Option(
        False, "--fresh", help="Ignore cached config, re-run from scratch"
    ),
    user: Optional[str] = typer.Option(None, "--user", help="WordPress admin username"),
    wp_pass: Optional[str] = typer.Option(
        None, "--pass", help="WordPress admin password"
    ),
    ...
    if not path.exists():
        console.print(
            f"[bold red]Error: Project path {path} does not exist.[/bold red]"
        )
        raise typer.Exit(code=1)
    ...
    if config_file.exists() and not fresh:
        ...
        skip_ai = not os.getenv("ANTHROPIC_API_KEY")
    ...
    # If saved config has skip_ai=False but no key, downgrade silently
    if not config.skip_ai and not os.getenv("ANTHROPIC_API_KEY"):
        config.skip_ai = True
    ...
    framework_label = (
        config.framework
        if isinstance(config.framework, str)
        else config.framework.value
    )
    ai_label = (
        f"Enabled ({config.ai_model})"
        if not config.skip_ai
        else "Off (Claude Code handles AI writing)"
    )
    console.print()
    console.print(f"  [bold cyan]KodaDocs {VERSION}[/bold cyan]")
    ...
    # Clear manifest when --fresh so all steps re-run
    if fresh:
    ...
    if do_deploy:
        ...
        orchestrator.register_step(
            "Deploy", deploy_step, critical=False, force_rerun=True
        )
...
def deploy_cmd(
    ...
    provider: Optional[str] = typer.Option(
        ...
        "--provider",
        "-p",
        help="Deploy provider (cloudflare, vercel, netlify, github-pages)",
    ),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Docs output directory override"
    ),
    license_key: Optional[str] = typer.Option(
        None, "--license-key", help="KodaDocs Pro license key"
    ),
    site_slug: Optional[str] = typer.Option(
        None, "--slug", help="Site subdomain for kodadocs.com hosting"
    ),
):
    ...
    if resolved is None:
        ...
        console.print(
            "Use [bold]--provider[/bold] to choose one: cloudflare, vercel, netlify, github-pages, kodadocs"
        )
        if detected_platform:
            console.print(
                f"Detected platform '{detected_platform}' is not a supported deploy target."
            )
        raise typer.Exit(code=1)
    ...
    if not dist_dir.is_dir():
        console.print(
            f"[bold red]Error: Build directory not found at {dist_dir}[/bold red]"
        )
...
def update(
    path: Path = typer.Argument(Path("."), help="Project path"),
    url: Optional[str] = typer.Option(None, "--url", "-u", help="Override app URL"),
    routes: Optional[str] = typer.Option(
        None,
        "--routes",
        "-r",
        help="Force re-capture specific routes (comma-separated, e.g., /dashboard,/settings)",
    ),
    skip_deploy: bool = typer.Option(
        True, "--skip-deploy/--deploy", help="Skip deployment"
    ),
):
    """Incrementally update docs — only re-captures and re-writes changed routes."""
    from .pipeline.update import (
        compute_route_diff,
    ...
    if not manifest:
        console.print("[bold red]No existing manifest found.[/bold red]")
        console.print(
            "Run [bold]kodadocs generate[/bold] first to create initial documentation."
        )
        raise typer.Exit(code=1)
    ...
    # Apply --url override
    if url:
        manifest.config.app_url = url

    # Parse forced routes
    forced_routes = [r.strip() for r in routes.split(",")] if routes else []

    # Save previous routes for diffing
    previous_routes = list(manifest.previous_routes or manifest.discovered_routes)

    # Print banner
    console.print()
    console.print(f"  [bold cyan]KodaDocs {VERSION}[/bold cyan] — Incremental Update")
    console.print(f"  Project: {path}")
    console.print(f"  Previous routes: {len(previous_routes)}")
    console.print()

    # Re-discover routes
    console.print("Re-discovering routes...")
    orchestrator.run_step_directly("UpdateDiscovery", discovery_step)
    ...
    if changed:
        console.print(f"  [yellow]Re-capturing:[/yellow] {', '.join(sorted(changed))}")
    ...
    # Prune removed routes and orphaned articles
    if removed:
        prune_removed_routes(manifest, removed)
        prune_removed_articles(manifest, removed)
        console.print(f"  Pruned data for {len(removed)} removed route(s).")

    # Selective capture and annotation for added + changed routes
    routes_to_process = added | changed
    if routes_to_process:
        console.print(f"Capturing {len(routes_to_process)} route(s)...")
        orchestrator.run_step_directly(
            "UpdateCapture", selective_capture_step, routes_to_process
        )

        console.print(f"Annotating {len(routes_to_process)} route(s)...")
        orchestrator.run_step_directly(
            "UpdateAnnotation", selective_annotation_step, routes_to_process
        )

    # Incremental enrichment for new/changed routes only
    if added or changed:
        console.print("Running incremental enrichment...")
        orchestrator.run_step_directly(
            "IncrementalEnrichment", incremental_enrichment_step, added, changed
        )

    # Always rebuild full VitePress output (cheap, needed for sidebar updates)
    console.print("Rebuilding documentation output...")
    orchestrator.run_step_directly("UpdateOutput", output_step)

    # Update previous_routes snapshot
    manifest.previous_routes = list(manifest.discovered_routes)
    ...
    if not skip_deploy:
        ...
        orchestrator.run_step_directly("UpdateDeploy", deploy_step)

    # Summary
    total_cost = sum(s.cost_estimate for s in manifest.steps.values())
    console.print()
    console.print("[bold green]Update complete![/bold green]")
    console.print(
        f"  Routes added: {len(added)}, removed: {len(removed)}, re-captured: {len(changed)}"
    )
    console.print(f"  Articles: {len(manifest.articles)} total")
    console.print(f"  Estimated cost: [bold cyan]${total_cost:.4f}[/bold cyan]")
